    ...
    bool                             projectToInput) const
{
  ...
  if (type == VALUE_NEAREST_NEIGHBOR) {
    ...
    configuredMapping.mapping = PtrMapping(
        new PartitionOfUnityMapping<CompactPolynomialC6>(constraintValue, dimensions, rbfParameter.value, polynomial, verticesPerPartition, relativeOverlap, projectToInput));
    configuredMapping.isRBF = false;
    return configuredMapping;
  }
