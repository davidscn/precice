...
bool compatibleTimeWindowSizes(const CouplingScheme &impl, const CouplingScheme &expl)
{
  ...
  if (math::equals(edt, idt)) {
    return true; // equal is compatible
  }
  if (math::smaller(edt, idt)) {
    return false; // smaller edt is incompatible
  }
  return math::equals(std::remainder(edt, idt), 0.0);
}
} // namespace
...
bool CompositionalCouplingScheme::addComputedTime(double timeToAdd)
{
  ...
  // If the implicit scheme is iterating, then the are done.
  if (auto isIterating = _explicitOnHold;
      isIterating) {
